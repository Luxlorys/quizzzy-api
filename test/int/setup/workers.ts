 ...
 * test files may run at once. global.ts creates exactly this many databases
 * and vitest is capped to the same number of workers, so every worker owns
 * one database for the whole run (see env.ts).
 */
export const INT_TEST_WORKERS = Number(
    process.env.INT_TEST_WORKERS ?? Math.max(1, Math.min(os.cpus().length - 1, 8)),
);
